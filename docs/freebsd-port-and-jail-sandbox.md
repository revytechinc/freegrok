...
| 2026-07-21 | FreeBSD 15.1 freedev007 | x86_64-unknown-freebsd 1.96.1 | **installed-app regression 29/0** | `scripts/freebsd-installed-regression.sh`: make build/doctor/install → test installed `grok-build` (version, doctor --ci, inspect, jail, mcp, completions, docs, uninstall/reinstall). pass=29 fail=0. |
| 2026-08-12 | Linux + freedev007 15.1 + freedev008 16.0 | host / 1.96.1 / 1.97.1 | **full pkg+integration+matrix** | `0ae8323`. Linux sandbox all-targets 98+2+8+5+1. Jail helper: `GROK_JAIL_HELPER` set-but-missing fail-closed; lib 53+helper 2 on 15+16. Pkg-regression **18/0** on 007 (helper dry-run + apply fail-closed). Product integration + L6 PONG/CANARY_OK on 007 (default) and 008 (`-m minimax-direct-m3`). Share SHA HTTP-verified 15=`c865856b…` 73M / 16=`f73a26f9…` 84M. 007 cargo matrix END pass=44 fail=4 (sandbox helper test — fixed; pager ZFS `st_blocks` — fixed+retest 4/4; shell SIGSEGV — `RUST_MIN_STACK=32M` retest green; workspace 11 permission tests unwrap/HOME). 008 matrix in progress; hooks `stop_collects_all_blocks` flaked at 5s, retest ok @ 30s. |
