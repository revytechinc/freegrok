...
| 2026-08-11 | FreeBSD 15.1 freedev007 + FreeBSD 16.0 freedev008; Linux authoring | rustc 1.96.1 / 1.97.1 / host | **upstream merge red–green + factory + pkg** | Branch `merge/upstream-be71313`: `SandboxBackendFactory::for_os` RED→GREEN (FreeBSD never NonoLinux; bare `unix` → Noop). Doctor reports `Sandbox backend: jail` via `platform_backend_kind()`. FreeBSD voice stub (`capture_unsupported`). Sandbox lib 98 (Linux) / 52 (15+16). `cargo check -p xai-grok-pager-bin` green 15+16; release builds green. Package `grok-build-0.2.106_3` + `pkg install`; `pkg which` → package; `doctor --ci` OK backend jail. Installed regression pass=25 fail=0. `scripts/freebsd-pkg-regression.sh` pass=13 fail=0 on 007. 008: userspace release binary doctor jail (ports tree not on host). |
