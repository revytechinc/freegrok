...
"#,
    ...
    // Prefer FreeBSD ports bash, then Linux /bin/bash, then PATH.
    let bash_candidates = [
        "/usr/local/bin/bash",
        "/bin/bash",
        "/usr/bin/bash",
    ...
    for bash in bash_candidates {
        ...
        match run_with_deadline(bash_cmd, deadline, bash) {
            RunOutcome::Completed { output: o, .. } if o.status.success() => {
                output = Some(o);
                break;
            }
            RunOutcome::Completed { .. } => {
                saw_fail = true;
                tracing::debug!(?envrc_path, bash, "bash .envrc non-success; trying next");
            }
