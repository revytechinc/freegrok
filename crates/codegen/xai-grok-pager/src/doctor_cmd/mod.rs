...
pub struct DoctorArgs {
    ...
    #[arg(long)]
    pub json: bool,
    /// Critical subset only (faster). FreeBSD platform doctor path.
    #[arg(long)]
    pub quick: bool,
    /// Treat warnings as failures.
    #[arg(long)]
    pub strict: bool,
    /// Build/CI gate: offline + quick; fail only on critical (FreeBSD ports/regression).
    #[arg(long)]
    pub ci: bool,
    /// Allow network probes (platform doctor).
    #[arg(long)]
    pub online: bool,
    /// Use credentials / models probes (platform doctor).
    #[arg(long)]
    pub auth: bool,
    /// Run MCP connectivity checks (platform doctor).
    #[arg(long)]
    pub mcp: bool,
    /// Sandbox-deep checks (FreeBSD helper; never prompts sudo).
    #[arg(long)]
    pub sandbox_deep: bool,
    /// One live inference turn (platform doctor; may be unimplemented).
    #[arg(long)]
    pub live: bool,
    #[command(subcommand)]
...
impl DoctorArgs {
    /// True when FreeBSD/platform readiness path should run instead of terminal-only doctor.
    pub fn wants_platform_doctor(&self) -> bool {
        self.ci
            || self.quick
            || self.strict
            || self.online
