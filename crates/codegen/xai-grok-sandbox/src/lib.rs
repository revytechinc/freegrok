...
//! FreeBSD: jail net params planned).
//!
//! The `enforce` feature (on by default) enables:
//! - **Linux / macOS:** [nono](https://crates.io/crates/nono) (Landlock/Seatbelt),
//!   bwrap re-exec, and hook write-deny
//! - **FreeBSD:** jail re-exec (`jail` module; helper-backed)
//!
...
pub use backend::{
    SandboxBackendFactory, SandboxBackendKind, host_backend_kind, platform_backend_kind,
};
...
/// FreeBSD twin of bwrap re-exec. Always `None` on non-FreeBSD until helper exists.
...
/// FreeBSD jail helper discovery + backend status (doctor / `grok jail`).
#[cfg(target_os = "freebsd")]
pub use jail::{
    JAIL_ENV_VAR, JAIL_HELPER_ENV, JailBackendStatus, jail_backend_status, resolve_jail_helper,
};
...
#[cfg(not(target_os = "freebsd"))]
pub fn resolve_jail_helper() -> Option<std::path::PathBuf> {
    None
}

/// Portable snapshot when not on FreeBSD (doctor still asks for a string).
#[cfg(not(target_os = "freebsd"))]
#[derive(Debug, Clone)]
...
/// Single-line / multi-line status for doctor when jail is N/A.
pub fn jail_backend_status_string() -> String {
    let s = jail_backend_status();
    format!(
        "inside_jail={} sysctl_jailed={:?} marker_env={} helper={}",
        s.inside_jail,
        s.sysctl_jailed,
        s.marker_env,
        s.helper
...
impl SandboxManager {
    ...
    #[cfg(not(any(all(feature = "enforce", any(target_os = "linux", target_os = "macos")), all(feature = "enforce", target_os = "freebsd"))))]
